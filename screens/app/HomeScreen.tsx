...
const HomeScreen: React.FC = () => {
  ...
  const categoriaAtivaId = useSelector((s: RootState) => s.categorias.categoriaAtivaId);
  ...
  const stats = [
    ...
    { id: 'agendado', label: 'Agendados', value: countAgendados, color: '#FF9F0A' }, // Laranja
    { id: 'concluido', label: 'Concluídos', value: countConcluidos, color: '#30D158' }, // Verde
    { id: 'todos', label: 'Todos', value: countTodos, color: '#0A84FF' },    // Azul
  ];
            ...
            {categoriasComTodos.map(c => {
              ...
              return (
                <CategoryChip
                  key={id}
                  label={c.nome}
                  active={!deleteMode && !editMode && id === String(categoriaAtivaId)}
...
const styles = StyleSheet.create({
  ...
  logoutText: { color: '#ff6b6b', fontWeight: 'bold', fontSize: 14 }, // Adicionado
